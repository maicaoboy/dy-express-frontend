  ...
  methods: {
    do_polygon: function() {
      ...
      this.map.addEventListener('click', function(e) {
        ...
        console.log(marker)
        that.polygon_marker.push(marker)
    ...
    show_polygon() {
      ...
      if (this.fence_polygon) {
        // 已经存在多边形,进行更新
        this.fence_polygon.setPath(points)
      } else {
        // 创建多边形围栏
        // eslint-disable-next-line no-undef
        this.fence_polygon = new BMap.Polygon(points, {
          strokeColor: 'blue',
          strokeWeight: 2,
          strokeOpacity: 0.5
        })
        this.map.addOverlay(this.fence_polygon)
      }
    },
    clear() {
