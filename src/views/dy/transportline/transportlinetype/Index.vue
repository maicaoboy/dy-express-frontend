<template>
  <div class="app-container">
    ...
    <el-form :inline="true" :label-position="labelPosition" label-width="80px" :model="formdata">
      <el-row>
        <el-col :span="8">
          <el-form-item :inline="true" :label="$t('table.transportlinetype.typeNumber')">
            <el-input v-model="formdata.typeNumber" :placeholder="$t('table.transportlinetype.searchtypeNumber')" />
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item :label="$t('table.transportlinetype.name')">
            <el-input v-model="formdata.name" :placeholder="$t('table.transportlinetype.searchname')" />
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item :label="$t('table.transportlinetype.agencyType')">
            <el-select v-model="formdata.agencyType" :placeholder="$t('table.transportlinetype.searchagencyType')">
              <el-option
...
import TransportlineTypeEdit from './Edit'

export default {
  components: { Pagination, TransportlineTypeEdit },
  data() {
    return {
      dialog: {
        isVisible: false,
  ...
  methods: {
    ...
    fetch(params = {}) {
      ...
      // console.log(params)
      transportlinetypeApi.page(params).then(response => {
    ...
    handleEdit(row) {
      console.log(row)
      this.$refs.edit.setTransportlinetype(row)
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除该路线类型，是否继续?', '提示', {
        ...
        return transportlinetypeApi.delete(row.id)
      }).then(response => {
        const res = response.data
        if (res.isSuccess) {
          this.$message({
